...
More investigation is needed to see if the divergence measures are robust when data distribution is less similar to a Gaussian distribution. Moreover, the method should also be compared to other feature selection methods. Finally, an improvement could be made to increase the stability of the selected features. For example, a selection of continuous intervals and not band is a possible solution which has already been explored in \cite{serpico2007extraction}.
